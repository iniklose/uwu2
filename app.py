...
def setup_wallet():
    ...
    try:
        res = subprocess.run(["npx", "-y", "naracli@latest", "address"], capture_output=True, text=True, timeout=15)
...
def sync_blockchain_balance():
    try:
        res = subprocess.run(["npx", "-y", "naracli@latest", "balance"], capture_output=True, text=True, timeout=20)
...
def get_quest_json():
    ...
    try:
        res = subprocess.run(["npx", "-y", "naracli@latest", "quest", "get", "--json"],
...
def is_free_tier(quest_data):
    ...
    if match:
        try:
            ...
            # Jika stake <= MAX_STAKE (0.1), lanjut.
            if stake <= MAX_STAKE:
                return True
                
            # SPECIAL FIX: Jika stakeRequirement sama persis dengan stakeLow, 
            # ada kemungkinan itu hanya saran/default dari node yang lag.
            # Kita coba anggap free jika angkanya mencurigakan (opsional).
            # return True if stake == float(match_low.group(1)) else False
...
def submit_answer(answer):
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            res = subprocess.run(["npx", "-y", "naracli@latest", "quest", "answer", answer],
